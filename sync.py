...
def write_to_file(_date, flights):
    filename = '/tmp/sync-{}.csv'.format(_date)
...
for j in list2:
    values.append(f'{j.div.div.div.div.div.div.contents[0].text}{j.div.div.div.div.div.div.contents[1].text}' if len(j.div.div.div.div.div.div.contents) == 2 else f'{j.div.div.div.div.div.div.contents[0].text}')
for k in range(0, len(list1)):
    ...
    if(v == None):
        continue
    if(values[k] == "No data"):
        datas.update({f"{titles[k]}" : f"{values[k]} - Check"})
    elif(values[k].split(" ")[0] == "OFF"):
        datas.update({f"{titles[k]}" : f"{values[k]} - OK"})
    elif((float(values[k].split(" ")[0]) >= 1 and values[k].split(" ")[1] == "day") or (float(values[k].split(" ")[0]) >= 1.0 and values[k].split(" ")[1] == "week")):
        datas.update({f"{titles[k]}" : f"{values[k]} - Check"})
    else:
        datas.update({f"{titles[k]}" : f"{values[k]} - OK"})
print("---------")
